...
// 임시 질문 데이터
const mockQuestions: question.RandomlySelectInterviewQuestionByFiltersResponse[] = [
  {
    id: 1,
...
export async function POST(request: NextRequest) {
  try {
    const body: question.RandomlySelectInterviewQuestionByFiltersRequest = await request.json();
    const { category, company, year } = body;
